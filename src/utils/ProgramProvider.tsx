...
export const ProgramProvider: FC<ProgramProviderProps> = ({ children }) => {
	...
	const getPoolIdoData = async () => {
		try {
			...
			if (res?.content?.dataType === 'moveObject') return res.content.fields;
			else return null;
		} catch (err) {
			return null;
		}
	...
	const ido_commit = useCallback(
		async (commitAmount: number) => {
			...
			if (coins.length > 1) {
				tx.mergeCoins(
					tx.gas,
					coins.slice(0, coins.length).map((item) => {
			...
			tx.moveCall({
				...
				typeArguments: [InfoIdo.token.address],
				arguments: [
	...
	const ido_claim = useCallback(async () => {
		...
		tx.moveCall({
			...
			typeArguments: [InfoIdo.token.address],
			arguments: [tx.object(InfoIdo.pool), tx.object('0x6')],
		});
		await wallet.signAndExecuteTransactionBlock({
			transactionBlock: tx as any,
		});
